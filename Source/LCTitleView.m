...
- (void)initTitleView
{
    ...
    for (int i = 0; i < _titles.count; i++) {
        ...
        label.lineBreakMode = self.style.labelLineBreakMode;
...
- (void)calculateTitleWidths
{
    ...
    for (NSString *title in _titles) {
        ...
        if (textWidth > _maxWidth) {
            if (self.style.isAverageTitleWidth) {
                if (textWidth > self.lc_width / _titles.count) {
                    textWidth = self.lc_width / _titles.count;
                }
            }
            _maxWidth = textWidth;
        }
